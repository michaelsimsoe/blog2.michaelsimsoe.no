class ArticlePreview extends HTMLElement {
    ...
    render() {
      ...
      const { title, date, updated, description, tags, readingDuration, slug } = this._data;
        ...
        <article>
          <header class="article-preview">
            ...
            <div class="article-preview__tags">
              ${tags.map(tag => `<div class="article-preview__tag"><a href="#">${tag}</a></div>`).join('')}
            </div>
            <div class="article-preview__meta">
              <time datetime="${new Date(date).toISOString()}">
                ${new Date(date).toLocaleDateString()} <em>(oppdatert ${new Date(updated).toLocaleDateString()})</em>
              </time>
